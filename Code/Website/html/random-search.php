<?php
session_start();
include('database_connection.php');//Including our DB Connection file
...
$random = rand(1,2600);
$query = "SELECT repo_name, username, language, readme FROM repos WHERE id = $random";
...
if($result){//If query successfull
 ...
 while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)){ //Display the record
 	...
 	$URL=$_SESSION['r_name'];
 	$urlDisplay = '<p><a href=/randomTempStorage.php?'.$URL.' <b>'.$row['repo_name'].'</a></b> '.'</p>';
 	echo $urlDisplay;
 	echo $readme;
